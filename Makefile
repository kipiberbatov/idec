...
ARRAY_LDLIBS := build/$(BUILD_TYPE)/lib/libarray$(.LIB)
ALGEBRA_LDLIBS := build/$(BUILD_TYPE)/lib/libalgebra$(.LIB) $(ARRAY_LDLIBS)
REGION_LDLIBS := build/$(BUILD_TYPE)/lib/libregion$(.LIB) $(ARRAY_LDLIBS)
MESH_LDLIBS := build/$(BUILD_TYPE)/lib/libmesh$(.LIB)\
  build/$(BUILD_TYPE)/lib/libregion$(.LIB) $(ALGEBRA_LDLIBS)
GRAPHICS_LDLIBS := build/$(BUILD_TYPE)/lib/libgraphics$(.LIB) $(MESH_LDLIBS)
SHARED_LDLIBS := build/$(BUILD_TYPE)/lib/libshared$(.DLL) $(MESH_LDLIBS)
...
build/$(BUILD_TYPE)/obj: | build/$(BUILD_TYPE)
	$(MKDIR) -p $@

# array
ARRAY_OBJ_NAMES := $(wildcard code/c/src/array/*$(.SRC))
ARRAY_OBJ := $(patsubst code/c/src/array/%$(.SRC),\
  build/$(BUILD_TYPE)/obj/%$(.OBJ), $(ARRAY_OBJ_NAMES))

.PHONY: obj_array
obj_array: $(ARRAY_OBJ)

$(ARRAY_OBJ): build/$(BUILD_TYPE)/obj/%$(.OBJ): code/c/src/array/%$(.SRC)\
  | build/$(BUILD_TYPE)/obj
...
ALGEBRA_OBJ := $(patsubst code/c/src/algebra/%$(.SRC),\
  build/$(BUILD_TYPE)/obj/%$(.OBJ), $(ALGEBRA_OBJ_NAMES))

.PHONY: obj_algebra
obj_algebra: $(ALGEBRA_OBJ)

$(ALGEBRA_OBJ): build/$(BUILD_TYPE)/obj/%$(.OBJ): code/c/src/algebra/%$(.SRC)\
    | build/$(BUILD_TYPE)/obj
...
REGION_OBJ := $(patsubst code/c/src/region/%$(.SRC),\
  build/$(BUILD_TYPE)/obj/%$(.OBJ), $(REGION_OBJ_NAMES))

.PHONY: obj_region
obj_region: $(REGION_OBJ)

$(REGION_OBJ): build/$(BUILD_TYPE)/obj/%$(.OBJ): code/c/src/region/%$(.SRC)\
    | build/$(BUILD_TYPE)/obj
...
MESH_OBJ := $(patsubst code/c/src/mesh/%$(.SRC),\
  build/$(BUILD_TYPE)/obj/%$(.OBJ), $(MESH_OBJ_NAMES))

.PHONY: obj_mesh
obj_mesh: $(MESH_OBJ)

$(MESH_OBJ): build/$(BUILD_TYPE)/obj/%$(.OBJ): code/c/src/mesh/%$(.SRC)\
    | build/$(BUILD_TYPE)/obj
...
GRAPHICS_OBJ := $(patsubst code/c/src/graphics/%$(.SRC),\
  build/$(BUILD_TYPE)/obj/%$(.OBJ), $(GRAPHICS_OBJ_NAMES))

.PHONY: obj_graphics
obj_graphics: $(GRAPHICS_OBJ)

$(GRAPHICS_OBJ): build/$(BUILD_TYPE)/obj/%$(.OBJ): code/c/src/graphics/%$(.SRC)\
    | build/$(BUILD_TYPE)/obj
...
SHARED_OBJ := $(patsubst code/c/src/shared/%$(.SRC),\
  build/$(BUILD_TYPE)/obj/%$(.OBJ), $(SHARED_OBJ_NAMES))

.PHONY: obj_shared
obj_shared: $(SHARED_OBJ)

$(SHARED_OBJ): build/$(BUILD_TYPE)/obj/%$(.OBJ): code/c/src/shared/%$(.SRC)\
    | build/$(BUILD_TYPE)/obj
...
############################# header dependencies ##############################
-include build/$(BUILD_TYPE)/obj/*$(.DEP)

# array
ARRAY_HEADER_DEP := $(patsubst code/c/src/array/%$(.SRC),\
  build/$(BUILD_TYPE)/obj/%$(.DEP), $(ARRAY_OBJ_NAMES))

# algebra
ALGEBRA_HEADER_DEP := $(patsubst code/c/src/algebra/%$(.SRC),\
  build/$(BUILD_TYPE)/obj/%$(.DEP), $(ALGEBRA_OBJ_NAMES))

# region
REGION_HEADER_DEP :=\
  $(patsubst code/c/src/region/%$(.SRC), build/$(BUILD_TYPE)/obj/%$(.DEP),\
    $(REGION_OBJ_NAMES))

# mesh
MESH_HEADER_DEP := $(patsubst code/c/src/mesh/%$(.SRC),\
  build/$(BUILD_TYPE)/obj/%$(.DEP), $(MESH_OBJ_NAMES))

# graphics
GRAPHICS_HEADER_DEP := $(patsubst code/c/src/graphics/%$(.SRC),\
  build/$(BUILD_TYPE)/obj/%$(.DEP), $(GRAPHICS_OBJ_NAMES))

# shared
SHARED_HEADER_DEP := $(patsubst code/c/src/shared/%$(.SRC),\
  build/$(BUILD_TYPE)/obj/%$(.DEP), $(SHARED_OBJ_NAMES))

################################## archiving ###################################
build/$(BUILD_TYPE)/lib: | build/$(BUILD_TYPE)
	$(MKDIR) -p $@

# array
.PHONY: lib_array
lib_array: build/$(BUILD_TYPE)/lib/libarray$(.LIB)

build/$(BUILD_TYPE)/lib/libarray$(.LIB): $(ARRAY_OBJ)\
  | build/$(BUILD_TYPE)/lib
	$(AR) $(ARFLAGS) $@ $^

# algebra
.PHONY: lib_algebra
lib_algebra: build/$(BUILD_TYPE)/lib/libalgebra$(.LIB)

build/$(BUILD_TYPE)/lib/libalgebra$(.LIB): $(ALGEBRA_OBJ)\
  | build/$(BUILD_TYPE)/lib
	$(AR) $(ARFLAGS) $@ $^

# region
.PHONY: lib_region
lib_region: build/$(BUILD_TYPE)/lib/libregion$(.LIB)

build/$(BUILD_TYPE)/lib/libregion$(.LIB):\
 $(REGION_OBJ) | build/$(BUILD_TYPE)/lib
	$(AR) $(ARFLAGS) $@ $^

# mesh
.PHONY: lib_mesh
lib_mesh: build/$(BUILD_TYPE)/lib/libmesh$(.LIB)

build/$(BUILD_TYPE)/lib/libmesh$(.LIB): $(MESH_OBJ) | build/$(BUILD_TYPE)/lib
	$(AR) $(ARFLAGS) $@ $^

# graphics
.PHONY: lib_graphics
lib_graphics: build/$(BUILD_TYPE)/lib/libgraphics$(.LIB)\
  | build/$(BUILD_TYPE)/lib

build/$(BUILD_TYPE)/lib/libgraphics$(.LIB): $(GRAPHICS_OBJ)\
  | build/$(BUILD_TYPE)/lib
	$(AR) $(ARFLAGS) $@ $^

#shared
.PHONY: lib_shared
lib_shared: build/$(BUILD_TYPE)/lib/libshared$(.DLL)

build/$(BUILD_TYPE)/lib/libshared$(.DLL):\
  $(SHARED_OBJ) $(ALGEBRA_OBJ) $(ARRAY_OBJ) | build/$(BUILD_TYPE)/lib
	$(CC) -o $@ -fPIC -shared $^
...
################################### bining ####################################
build/$(BUILD_TYPE)/bin: | build/$(BUILD_TYPE)
	-$(MKDIR) -p $@

# array
ARRAY_EXE_NAMES := $(wildcard code/c/main/array/*$(.SRC))
ARRAY_EXE := $(patsubst code/c/main/array/main_%$(.SRC),\
  build/$(BUILD_TYPE)/bin/%$(.EXE), $(ARRAY_EXE_NAMES))

.PHONY: bin_array
bin_array: $(ARRAY_EXE)

$(ARRAY_EXE): build/$(BUILD_TYPE)/bin/%$(.EXE):\
 code/c/main/array/main_%$(.SRC) $(ARRAY_LDLIBS) | build/$(BUILD_TYPE)/bin
...
ALGEBRA_EXE := $(patsubst code/c/main/algebra/main_%$(.SRC),\
  build/$(BUILD_TYPE)/bin/%$(.EXE), $(ALGEBRA_EXE_NAMES))

.PHONY: bin_algebra
bin_algebra: $(ALGEBRA_EXE)

$(ALGEBRA_EXE): build/$(BUILD_TYPE)/bin/%$(.EXE):\
  code/c/main/algebra/main_%$(.SRC) $(ALGEBRA_LDLIBS) | build/$(BUILD_TYPE)/bin
...
REGION_EXE := $(patsubst code/c/main/region/main_%$(.SRC),\
  build/$(BUILD_TYPE)/bin/%$(.EXE), $(REGION_EXE_NAMES))

.PHONY: bin_region
bin_region: $(REGION_EXE)

$(REGION_EXE): build/$(BUILD_TYPE)/bin/%$(.EXE):\
  code/c/main/region/main_%$(.SRC) $(REGION_LDLIBS) | build/$(BUILD_TYPE)/bin
...
MESH_EXE := $(patsubst code/c/main/mesh/main_%$(.SRC),\
  build/$(BUILD_TYPE)/bin/%$(.EXE), $(MESH_EXE_NAMES))

.PHONY: bin_mesh
bin_mesh: $(MESH_EXE)

$(MESH_EXE): build/$(BUILD_TYPE)/bin/%$(.EXE):\
  code/c/main/mesh/main_%$(.SRC) $(MESH_LDLIBS) | build/$(BUILD_TYPE)/bin
...
GRAPHICS_EXE := $(patsubst code/c/main/graphics/main_%$(.SRC),\
  build/$(BUILD_TYPE)/bin/%$(.EXE), $(GRAPHICS_EXE_NAMES))

.PHONY: bin_graphics
bin_graphics: $(GRAPHICS_EXE)

$(GRAPHICS_EXE): build/$(BUILD_TYPE)/bin/%$(.EXE):\
...
-include build/$(BUILD_TYPE)/bin/*$(.DEP)

# array
ARRAY_OBJ_DEP := $(patsubst build/$(BUILD_TYPE)/bin/%$(.EXE),\
  build/$(BUILD_TYPE)/bin/%$(.DEP), $(ARRAY_EXE_NAMES))

# algebra
ALGEBRA_OBJ_DEP := $(patsubst build/$(BUILD_TYPE)/bin/%$(.EXE),\
  build/$(BUILD_TYPE)/bin/%$(.DEP), $(ALGEBRA_EXE_NAMES))

# region
REGION_OBJ_DEP := $(patsubst build/$(BUILD_TYPE)/bin/%$(.EXE),\
  build/$(BUILD_TYPE)/bin/%$(.DEP), $(REGION_EXE_NAMES))

# mesh
MESH_OBJ_DEP := $(patsubst build/$(BUILD_TYPE)/bin/%$(.EXE),\
  build/$(BUILD_TYPE)/bin/%$(.DEP), $(MESH_EXE_NAMES))

# graphics
GRAPHICS_OBJ_DEP := $(patsubst build/$(BUILD_TYPE)/bin/%$(.EXE),\
  build/$(BUILD_TYPE)/bin/%$(.DEP), $(GRAPHICS_EXE_NAMES))

# graphics
SHARED_OBJ_DEP := $(patsubst build/$(BUILD_TYPE)/bin/%$(.EXE),\
  build/$(BUILD_TYPE)/bin/%$(.DEP), $(SHARED_EXE_NAMES))

################################ running demos #################################
build/$(BUILD_TYPE)/demo: | build/$(BUILD_TYPE)
	mkdir -p $@
...
lib_array_clean:
	-$(RM) build/$(BUILD_TYPE)/lib/libarray$(.LIB)
...
lib_algebra_clean:
	-$(RM) build/$(BUILD_TYPE)/lib/libalgebra$(.LIB)
...
lib_region_clean:
	-$(RM) build/$(BUILD_TYPE)/lib/libregion$(.LIB)
...
lib_mesh_clean:
	-$(RM) build/$(BUILD_TYPE)/lib/libmesh$(.LIB)
...
lib_graphics_clean:
	-$(RM) build/$(BUILD_TYPE)/lib/libgraphics$(.LIB)
...
lib_shared_clean:
	-$(RM) build/$(BUILD_TYPE)/lib/libshared$(.LIB)
...
.PHONY: obj_clean
obj_clean:
	-$(RM) -r build/$(BUILD_TYPE)/obj

.PHONY: lib_clean
lib_clean:
	-$(RM) -r build/$(BUILD_TYPE)/lib

.PHONY: bin_clean
bin_clean:
	-$(RM) -r build/$(BUILD_TYPE)/bin
