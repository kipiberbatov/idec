...
\begin{discussion}
  ...
  Namely, if
    $X$ is a type (set),
    $\{Y(x)\}_{x \in X}$ ia a family of sets and
    $\{f(x) \in Y(x)\}_{x \in X}$,
  we will write
  \begin{equation}
...
\begin{formulation}
  ...
  \begin{equation}
    \begin{split}
      ...
      &
      \begin{cases}
        \left.(\frac{\partial (\pi_0 u)}{\partial t})\right|_{K_0'} =
...
%
        \left.
          \left(\overline{(\pi_1 \circ \delta_0) u}\right)
        \right|_{\Gamma_N} \cdot {\bf n} = g_N
