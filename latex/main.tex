...
\begin{discussion}
  ...
  I will use notation from dependent type theory for functions whose codomain
  depends on the domain.
  Namely, if
    $X$ is a type (set),
    $\{Y(x)\}_{x \colon X}$ ia a family of sets and
    $\{f(x) \colon Y(x)\}_{x \colon X}$,
  we will write
  \begin{equation}
    f \colon \prod_{x \colon X} Y(x).
  \end{equation}
  Let
    $d \in \N$,
  ...
  \begin{itemize}
    ...
    \item
      $\displaystyle
        \mathcal{L} \colon \prod_{x_0 \colon K_0} M_{n(x_0), d}(\R)$
...
\begin{example}
  Let
    ...
    $K$ be its Forman subdivision,
    $K' := K \setminus \partial K$
    $t_0 \in \R$,
    $u_0 \in C^0 K$,
    $f \in C^0 K' $
    $\Gamma_D, \Gamma_N$ form a partition of $K_0$,
    ${\bf n} \colon \Gamma_N \to \R^d$,
    $g_D \colon \Gamma_D \to \R$,
    $g_N \colon \Gamma_N \to \R$,
  ...
  \begin{equation}
    \begin{split}
      ...
      &
      \begin{cases}
        \left.(\frac{\partial (\pi_0 u)}{\partial t})\right|_{K'} =
        \left.((\delta_1^\star \circ \pi_1 \circ \delta_0) u)\right|_{K'} + f
...
%
        \tr_{\Gamma_N} \overline{(\pi_1 \circ \delta_0) u} \cdot {\bf n} = g_N
