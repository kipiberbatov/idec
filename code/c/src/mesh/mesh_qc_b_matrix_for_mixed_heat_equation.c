...
static void mesh_qc_b_matrix_for_mixed_heat_equation_set_cols_total(
  int * b_cols_total,
  const mesh_qc * m);

static void mesh_qc_b_matrix_for_mixed_heat_equation_set_row_indices(
  int * b_row_indices,
  const mesh_qc * m);

static void mesh_qc_b_matrix_for_mixed_heat_equation_set_value(
  double * b_values,
  const mesh_qc * m,
  const matrix_sparse ** m_bd);

matrix_sparse * mesh_qc_b_matrix_for_mixed_heat_equation(
  const mesh_qc * m, const matrix_sparse ** m_bd)
{
  int d, nonzero_max;
  int * m_cn;
  matrix_sparse * b;
  ...
  {
    fprintf(stderr,
      "%s:%d: cannot allocate memory for sparse mtrix b", __FILE__, __LINE__);
    goto end;
  }
  ...
  {
    fprintf(stderr,
      "%s:%d: cannot allocate memory for b->cols_total", __FILE__, __LINE__);
    goto b_free;
  }
  mesh_qc_b_matrix_for_mixed_heat_equation_set_cols_total(b->cols_total, m);
  ...
  {
    fprintf(stderr,
      "%s:%d: cannot allocate memory for b->row_indices", __FILE__, __LINE__);
    goto b_cols_total_free;
  }
  mesh_qc_b_matrix_for_mixed_heat_equation_set_row_indices(b->row_indices, m);

  b->values = (double *) malloc(sizeof(double) * nonzero_max);
  if (b->values == NULL)
  {
    fprintf(stderr,
      "%s:%d: cannot allocate memory for b->values", __FILE__, __LINE__);
    goto b_row_indices_free;
  }
  mesh_qc_b_matrix_for_mixed_heat_equation_set_values(b->values, m, m_bd);
